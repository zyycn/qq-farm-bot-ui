...
<template>
  ...
  >
    <div class="flex items-center gap-2 text-sm a-color-text-tertiary">
      <span class="rounded-md px-2 py-0.5 font-medium a-color-primary-text a-bg-primary-bg"
        >{{ friendCount }} 好友</span
      >
      <span v-if="blacklistedCount" class="rounded-md px-2 py-0.5 a-color-text-secondary a-bg-fill-secondary">
        {{ blacklistedCount }} 屏蔽
      </span>
    </div>
